class WelcomeController < ApplicationController
  def index
  	...
  	@lost_item_ids = current_user.lost_items.collect{|item| [item.id,item.lost_item]} if user_signed_in?

  	@found_report_count = (user_signed_in?)? current_user.found_items.count : 0
  	@found_item_ids = current_user.found_items.collect{|item| [item.id,item.found_item]} if user_signed_in?
